...
if (isset($_POST['result']))
{
    ...
    $ekle = "INSERT INTO `user_urun` (`user_urun_id`, `urun_id`) VALUES ('$id', '2') ";
    mysqli_query($baglanti, $ekle);
    $adet_bul = "SELECT urun_adet FROM urun where urun_id = '2' ";
    ...
    $update_adet = "UPDATE urun SET urun_adet ='$urun_adet' Where urun_id = '2'";
...
<div class="orta kapsayici">
    <div class="bolum4 urun">
        <img src="\web proje\jpg\gomlek2.jpg" alt="Kapak resmi"  height="350">
       <div class=urun_bilgi >
            <h1>Ürün adı: Kareli gri gömlek</h1>
            <h1>Ürün fiyatı: 200 TL</h1>
            <h1>Kalan ürün adeti:
             <?php 
             $baglanti = mysqli_connect($host, $kullanici, $password, $vt);

             $getir = "SELECT urun_adet FROM `urun` WHERE urun_id = '2' " ;
                
             $yazdir = mysqli_query($baglanti, $getir);   
             if(!$yazdir){
                die(mysqli_error($baglanti));
             }
                if (mysqli_num_rows($yazdir) === 1) {
                    $rowData = mysqli_fetch_assoc($yazdir);
                        if($rowData["urun_adet"]>1){
                            echo $rowData["urun_adet"];
                            ?>
                            <form action="urun2.php" method="post" class="form" role="form" name="form">
